...
namespace Html2Markdown.Replacement
{
	internal static class HtmlParser
	{
		...
		internal static string ReplacePre(string html)
		{
		    ...
		    var nodes = doc.DocumentNode.Descendants("Pre").ToList();
		...
		{
			var doc = GetHtmlDocument(html);
		    var nodes = doc.DocumentNode.Descendants("img").ToList();
			if (nodes == null) return html;
			...
			nodes.Each(node =>
				{
					...
					var markdown = string.Format(@"![{0}]({1})", alt, src);
		...
		{
			var doc = GetHtmlDocument(html);
		    var nodes = doc.DocumentNode.Descendants("blockquote").ToList();
			if (nodes == null) return html;
